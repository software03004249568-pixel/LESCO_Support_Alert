...
class MonitorService : Service() {
    ...
    private val alertChannelId = "complaint_alert_high_v3"
    private val serviceChannelId = "complaint_monitor_service"
    ...
    override fun onCreate() {
        ...
        getSharedPreferences("config", MODE_PRIVATE)
            .edit().putBoolean("monitoringEnabled", true).apply()
        createChannel()
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        // Some OEMs (including Infinix/XOS) remove a task from Recents and also
        // stop its process. Ask AlarmManager to wake the restart receiver shortly.
        try {
    ...
    override fun onDestroy() {
        ...
        // Do not clear monitoringEnabled here; only the user's Pause action clears it.
        super.onDestroy()
    }
    ...
    private fun createChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // The persistent service notification is intentionally quiet.
